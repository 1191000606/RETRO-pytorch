...
class Encoder(nn.Module):
    def __init__(
        ...
        wrapper = partial(PreNorm, dim, norm_klass=norm_klass) if not post_norm else partial(PostNorm, dim,
        ...
        for layer_num in range(1, depth + 1):
            ...
            self.layers.append(nn.ModuleList([
                ...
                wrapper(Attention(dim=dim, context_dim=context_dim, dim_head=dim_head, heads=heads,
                                  dropout=attn_dropout)) if has_cross_attn else None,
    ...
    def __init__(
        ...
        for layer_num in range(1, depth + 1):
            ...
            self.layers.append(nn.ModuleList([
                ...
                wrapper(ChunkedCrossAttention(chunk_size=chunk_size, dim=dim, dim_head=dim_head, heads=heads,
                                              dropout=attn_dropout)) if has_cross_attn else None,
                wrapper(FeedForward(dim=dim, mult=ff_mult, dropout=ff_dropout)),
            ]))
...
class RETRO(nn.Module):
    def __init__(
        ...
        self.token_emb = nn.Embedding(num_tokens, enc_dim)  # nn.Embedding初始化为正态分布
